...
`2c4e8d0 Feat: trade sessions API functions (SessionOut, fetchActiveSession, startSession, closeSession)`
...
- ✅ ~~Миграция VPS~~ — выполнена 6 июня 2026
- ✅ ~~Часть 2 (фронт)~~ — выполнена 6 июня 2026
- **Часть 3 — журнал по сессиям** — `TradesHistory` сейчас плоский список; сгруппировать по сессиям + показать итоговый P&L закрытых. Эндпоинт `GET /api/sessions/{id}` (SessionDetail с trades[]) уже готов на бэкенде.
- **Очистка локальной БД** — тестовые сессии от curl-тестов 1B (с нулевым курсом / битыми цифрами); почистить не срочно (локалка в бой не идёт)
...
### Далее (по приоритету)
- **Торговые сессии 3 (журнал)** — группировка `TradesHistory` по сессиям + итоги P&L по закрытым
