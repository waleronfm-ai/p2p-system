...
### Следующее крупное — Торговые сессии

### Далее (по приоритету)
- **Торговые сессии** — стартовый капитал ₴, привязка сделок к сессии, отчёт реализовано/нереализовано/итого на закрытии, таблица `sessions` + `session_id` в `trades`
