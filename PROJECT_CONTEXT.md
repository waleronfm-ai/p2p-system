...
- Метки сделок: журнал пуст, проверить при первой сделке (нужна стрелка на нужном ТФ с нужной ценой)
- Точки opportunities: рынок выше порога входа (~44.10 > threshold 43.97), появятся при предложении ниже threshold
- На отфильтрованных объёмах (5к+) линии зубчатые — будущая задача (агрессивнее сглаживание / шире окно агрегации для мелких выборок)

### Торговые сессии — что осталось

- **Миграция VPS** — бэкап боевой `p2p.db` → `create_all` создаст `sessions` → `session_id` добавить вручную через `sqlite3` (`ALTER TABLE trades ADD COLUMN session_id INTEGER REFERENCES sessions(id);`). ALTER-логика в коммит не входила — выполнять руками на сервере. Бэкап обязателен перед любым шагом.
- **Часть 2 — фронт** — поле «Стартовый капитал ₴» + кнопка «Старт сессии», шапка активной сессии, кнопка «Закрыть» + карточка отчёта (реализовано / нереализовано / итого)
- **Часть 3 — журнал по сессиям** — история группируется по сессиям, общий учёт по закрытым
- **Очистка локальной БД** — сессии №1/2/3 и тестовые сделки с нулевым курсом; почистить перед реальным использованием
...
### Далее (по приоритету)
- **Торговые сессии 2 (фронт)** — UI: старт / активная шапка / закрытие + отчёт
